...
use Login\Zend\Validator\ConfirmPassword;
...
use Login\Form\RegistrationForm;
use Login\Form\RegistrationFilter;
use Login\Form\ForgottenPasswordForm;
use Login\Form\ForgottenPasswordFilter;
...
//-		$form = $this->getRegistrationForm($entityManager, $user);
		...
		$form->setHydrator(new DoctrineHydrator($entityManager,'Login\Entity\Users'));		
        ...
        if ($request->isPost()) {
			...
			$form->setData($request->getPost());
			 if ($form->isValid()) {
				...
				$entityManager->flush();				
				return $this->redirect()->toRoute('login/default', array('controller'=>'registration', 'action'=>'registration-success'));					
			}			 
		}
	...
	public function confirmEmailAction()
	{
		...
		try {
			$entityManager = $this->getServiceLocator()->get('doctrine.entitymanager.orm_default');
			$user = $entityManager->getRepository('Login\Entity\Users')->findOneBy(array('usrRegistrationToken' => $token)); // 
			$user->setUsrActive(1);
			...
			$viewModel->setTemplate('login/registration/confirm-email-error.phtml');
		}
		return $viewModel;
	}
        ...
        if ($request->isPost()) {
			...
			$form->setData($request->getPost());
			 if ($form->isValid()) {
				...
				$entityManager = $this->getServiceLocator()->get('doctrine.entitymanager.orm_default');
				$user = $entityManager->getRepository('Login\Entity\Users')->findOneBy(array('usrEmail' => $usrEmail)); // 
				...
				$entityManager->persist($user);
				$entityManager->flush();				
                return $this->redirect()->toRoute('login/default', array('controller'=>'registration', 'action'=>'password-change-success'));
			}					
		}		
		return new ViewModel(array('form' => $form));			
	}
